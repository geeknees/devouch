...
実装追記（2026-09-26）: ENS推薦版のローカル実装・テストとGitHub CIが成功し、本人walletのSepolia公開・CLI方針比較を [検証済み](demo-evidence.md)。repoとPagesの公開・固定Actionの匿名取得も [確認済み](release-evidence.md)。実Sepolia失効・推薦付き実fork PRは未実施、Worldは未統合。[提出草案](submission.md)と [実装状況](implementation-status.md)を現在の進捗として参照する。
...
7. 推薦者が直接失効させ、両方の CLI と同じ Action の再実行で `revoked / not_evaluated` を確認する。
