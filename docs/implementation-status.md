...
| 管理者・エージェント両名義の実 fork PR での有効推薦 | 対象 PR と Action run の URL | エージェント名義は完了。人間名義は未実施。失効のPR検証はユーザー指定で対象外 |
...
masusanouのPRは完了扱い。失効の機能とウォレット・CLIの確認範囲は別に記録する。
...
人間名義の推薦付き実PR、本人walletによる実失効とCLIでの確認が残る。
